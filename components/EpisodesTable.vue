<template>
  <el-table
    size="mini"
    :data="episodes.filter(data => !search || data.name.toLowerCase().includes(search.toLowerCase().trim()))"
    style="width: 100%">
    <el-table-column
      prop="episode"
      label="Episode">
      ...
      <template slot-scope="scope">
        <el-popover trigger="click" placement="top">
          <div class="popover">
            ...
            <div class="airdate">{{ scope.row.air_date }}</div>
            <CharactersTable :characters="filteredCharacters" />
          </div>
          <div slot="reference" class="name-wrapper" @click="getCharacters(scope.row.characters.map(r => r.split('character/')[1]))">
